...
/**
 * WorkingWithChildrenOrderConfirmationPage page object
 * 
 * @author pkumar350
 */
public class WorkingWithChildrenOrderConfirmationPage extends OrderConfirmationPage {

	@FindBy(how = How.XPATH, using = "//h1[contains(text(),'Confirmation - Working with children check')]")
	WebElement wwccConfirmationPageTitle;
	...
	public WorkingWithChildrenOrderConfirmationPage(WebDriver driver) {
		super(driver);
	}

	public void verifyWorkingWithChildrenConfirmationPage(List<Map<String, String>> pageData) {
		log.info("Reviewing Working with children check confirmation page.");
		...
		try {
			Assert.assertEquals("Confirmation - Working with children check", wwccConfirmationPageTitle.getText());
		} catch (Error e) {
		...
		try {
			Assert.assertEquals(pageData.get(2).get("PhoneNumber"), phoneNumber.getText());
		} catch (Error e) {
			parseErrorMessageLog(e.toString());
		}
