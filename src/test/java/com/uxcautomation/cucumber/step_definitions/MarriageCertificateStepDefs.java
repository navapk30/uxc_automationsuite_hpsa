...
public class MarriageCertificateStepDefs {
	...
	public MarriageCertificateStepDefs() {
		...
		listingPage = StepBaseSetup.getPageObjectForClass(DisplayProductListBirthDeathMarriagePage.class);
	...
	public void user_will_fills_the_Marriage_Details_form_with_valid_data(DataTable applicationDetails)
			throws Throwable {
		StepBaseSetup.updateSauceTestCaseNameMethod(
				CommonStepsDefs.scenario.getName() + ": user_will_fills_the_Marriage_Details_form_with_valid_data");
		List<Map<String, String>> PartnerData = applicationDetails.asMaps(String.class, String.class);
		pageData.addAll(PartnerData);
		checkoutPage.enterMarriageDetails(PartnerData);
	}

	@Then("^User fills the Partner 1 Details form with valid data$")
	public void user_will_fills_the_Partner1_Details_form_with_valid_data(DataTable Partner1Details) throws Throwable {
		StepBaseSetup.updateSauceTestCaseNameMethod(
		...
		pageData.addAll(PartnerData);
		checkoutPage.enterPartner1Details(PartnerData);
	}

	@Then("^User fills the Partner 2 Details form with valid data$")
	public void user_will_fills_the_Partner2_Details_form_with_valid_data(DataTable Partner2Details) throws Throwable {
		StepBaseSetup.updateSauceTestCaseNameMethod(
				CommonStepsDefs.scenario.getName() + ": user_fills_the_Payment_and_Billing_address_form");
		List<Map<String, String>> PartnerData = Partner2Details.asMaps(String.class, String.class);
		pageData.addAll(PartnerData);
		checkoutPage.enterPartner2Details(PartnerData);
	}

	@Then("^User reviews Marriage Certificate service information$")
	public void user_reviews_Learner_Driver_Partner1_service_information() throws Throwable {
		StepBaseSetup.updateSauceTestCaseNameMethod(
				CommonStepsDefs.scenario.getName() + ": user_will_fills_the_Partner1_Details_form_with_valid_data");
	...
	public void user_submits_the_Learner_Driver_Partner1_form() throws Throwable {
		StepBaseSetup.updateSauceTestCaseNameMethod(
				CommonStepsDefs.scenario.getName() + ": user_submits_the_Learner_Driver_Partner1_form");
		checkoutPage.placeOrder();
	}

	@Then("^User is redirected to Marriage Certificate confirmation page$")
	public void user_is_redirected_to_Marriage_Certificate_confirmation_page() throws Throwable {
		StepBaseSetup.updateSauceTestCaseNameMethod(CommonStepsDefs.scenario.getName()
				+ ":user_is_redirected_to_Marriage_Certificate_confirmation_page");
		checkoutPage.verifyIsConfirmationPageDisplayed();
	}

	@Then("^User verifies the information in confirmation page for Marriage Certificate Service$")
	...
	@Then("^User \"([^\"]*)\" the primary address in shipping address page  for Marriage Certificate Service$")
	...
	@Then("^User fills the Shipment/Pickup location form with valid data for Marriage Certificate Service$")
	...
	@Then("^User selects an \"([^\"]*)\" from the Shipping Method page  for Marriage Certificate Service$")
	...
	@Then("^User fills the Payment and Billing address form  for Marriage Certificate Service$")
